...
})({"ClpZA":[function(require,module,exports) {
"use strict";
...
module.bundle.HMR_BUNDLE_ID = "4baa217c5afbc889";
...
},{}],"ew5Y3":[function(require,module,exports) {
const WIDTH = 1200;
const HEIGHT = 1000;
...
document.body.append(angleDiv, angelSlider, znearDiv, znearSlider, zfarDiv, zfarSlider);
...
function draw() {
    ...
    const p1Final = {
        x: rp1.x + linesCenter.x,
        y: rp1.y + linesCenter.y
    };
    const p2Final = {
        x: rp2.x + linesCenter.x,
        y: rp2.y + linesCenter.y
    };
    ...
    ctx.fillText("tan(angle/2) = " + Math.tan(rAngle / 2).toFixed(3), 10, HEIGHT - 75);
    ctx.fillText("1/tan(angle/2) = " + (1 / Math.tan(rAngle / 2)).toFixed(3), 10, HEIGHT - 20);
}
draw();
angelSlider.addEventListener("input", ()=>draw());
znearSlider.addEventListener("input", ()=>draw());
zfarSlider.addEventListener("input", ()=>draw());
function getPointOnLineAwayFromCenter(linePoint, centerPoint, awayAmount) {
    return calculateIntersection(linePoint, centerPoint, {
        x: 0,
        y: centerPoint.y - awayAmount
    }, {
        x: WIDTH,
        y: centerPoint.y - awayAmount
    });
...
},{}]},["ClpZA","ew5Y3"], "ew5Y3", "parcelRequirefcd4")

//# sourceMappingURL=index.5afbc889.js.map
